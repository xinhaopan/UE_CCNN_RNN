...
if __name__ == "__main__":
    ...
    for startyear in years:
        ...
        dataset_path = "../traindata_0.75"
        inputs_size = [256,256,11]
        NUM_CLASSES = 2
        ...
        with open(os.path.join(dataset_path, "labels" + str(startyear) + "/Segmentation/train.txt"),"r") as f:
            train_lines = f.readlines()
        train_dataset   = DeeplabDataset(train_lines, inputs_size, NUM_CLASSES, dataset_path,startyear)
        gen = DataLoader(train_dataset, batch_size=Batch_size, num_workers=4, pin_memory=True,
                                        drop_last=True, collate_fn=deeplab_dataset_collate)
    
        with open(os.path.join(dataset_path, "labels" + str(startyear) + "/Segmentation/train.txt"),"r") as f:
            val_lines = f.readlines()
        val_dataset     = DeeplabDataset(val_lines, inputs_size, NUM_CLASSES, dataset_path,startyear)
        ...
        time_str   = time_str
        save_path  = dataset_path + "/logs/" + str(startyear) + "/loss_" + str(time_str)
