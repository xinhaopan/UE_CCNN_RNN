...
def fit_one_epoch(net, epoch, epoch_size, gen, Epoch, cuda,dataset_path,startyear):
    net = net.eval()
    with torch.no_grad():
        with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
            for iteration, batch in enumerate(gen):
                ...
                resultpath = dataset_path + '/np_save_' + str(startyear)
                if not os.path.exists(resultpath):
                    os.makedirs(resultpath)

                np.save(resultpath + '/' + names[0], save_numpy)
...
if __name__ == "__main__":
    ...
    years = [2015,2005,2000,1995,1990,1985]

    for startyear in years:
        path = "../traindata_0.75/logs/{}".format(str(startyear))
        ...
        print(startyear)
        fit_one_epoch(model, 1, epoch_size, gen, Interval_Epoch, Cuda,dataset_path,startyear)
