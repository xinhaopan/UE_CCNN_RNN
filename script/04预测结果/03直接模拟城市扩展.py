...
ResultPath = r"D:\Work\doing\CNN_RNN\data\beijing\simdata" + str(beginyear) + "_0"
path = r"D:\Work\doing\CNN_RNN\data\beijing\origindata\urban\1985_2017_2"

boundary_dir = r"D:\Work\doing\CNN_RNN\data\beijing\inputdata\boundary\boundary.tif" # 研究区范围，1为研究区，0为非研究区
boundary_arr = xr.open_rasterio(boundary_dir).data[0,:,:]

sui_arr = xr.open_rasterio(ResultPath + "/Suitability1_0.75_0.tif").data[0,:,:]

prability_arr = sui_arr


# urban_arr = xr.open_rasterio(r"D:\Work\doing\CNN_RNN\data\beijing\origindata\urban\1985_2017_2\urban" + str(endyear) + ".tif").data[0,:,:]
# endYearCount = 1879419 # np.sum(urban_arr == 1) # 模拟未来则直接输入未来的城市像元数量
...
raster = gdal.Open(r"D:\Work\doing\CNN_RNN\data\beijing\origindata\urban\1985_2017_2\urban" + str(beginyear) + ".tif")
...
urbanOld_arr[np.where(prability_arr>0.5)] = 1
ResultFile = ResultPath + "/" + "simurban" + str(endyear)  + "_1.tif"
...
workbook = Workbook()
save_file = ResultPath + "/" + "unet精度" + str(endyear) + ".xlsx"
